...
@Component({
  selector: 'app-create-studentPayment',
  templateUrl: './create-studentPayment.component.html',
  styleUrls: ['./create-studentPayment.component.css']
})
export class CreatestudentPaymentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    onChange(student:Student){
      ...
      this.studentPaymentService.getTotalPaymentsByStudent(student.id).subscribe(data=>{
        ...
        this.balanceAmount=this.totalAmount-data;
  ...
  save() {
    ...
    .subscribe(data => {console.log(data),this.commonService.showNotification('top','center',"Hurray! "+sessionStorage.getItem('username')+" - Your Fleet Log has been created successfully!."),this.studentPayment=new StudentPayment()}, error => {console.log(error),this.commonService.showErrorNotification('top','center',"Arrghh!! Something went wrong!." +error)});
